package hotpointgame.grole
{
   ...
   public class PlayerDoP extends PlayerDo
   {
      ...
      private var tempMapGunNameArr:Array = ["跑攻","跳冲","技能1","技能2","技能3","技能4","技能5","技能6","技能7","阶段1","阶段2","阶段3","阶段4"];
      
      private var tempAngerArr:Array = ["倒地","被打","冰冻","眩晕","水泡","束缚","石化","死亡","待机","跑","走","起身","跳","跳冲"];
      
      public function PlayerDoP()
      {
         super();
         var _loc1_:Class = LoaderManager.getSwfClass("Skin_man") as Class;
         var _loc2_:MovieClip = new _loc1_();
         currentState = GS.a1;
         this.vp = new VPlayerP(_loc2_);
      }
      ...
      {
         this.vp.mapToMap(param1);
      }
      
      override public function lostMapWeapon() : void
      {
         this.vp.lostMapWeapon();
      }
      ...
      private function keyControl(param1:CPlayer) : void
      {
         ...
         if(currentState == GS.a2)
         {
            if(_loc2_)
            {
               if(!(GS.a9998 == GM.levelm.curLevel.id || GS.a9997 == GM.levelm.curLevel.id || GS.a9994 == GM.levelm.curLevel.id || GM.levelm.curLevel.id > GS.a4000 && GM.levelm.curLevel.id <= GS.a4000 + GS.a7 || GM.levelm.curLevel.id > GS.a4000 && GM.levelm.curLevel.id <= GS.a4000 + GS.a7))
               {
                  ReliveC.open();
               }
               currentState = GS.a3;
         ...
         if(Boolean(GM.ckey.isKey("跳冲")) && this.chongFlag == 0 && Boolean(this.skillIsKeUser(param1,"跳冲")))
         {
            _loc3_ = "跳冲";
            this.changeActionByNameAndUpdate(param1,_loc3_);
         ...
         if(GM.ckey.isKey("攻击"))
         {
            ...
            if(param1.gunslot.getCurrentG() == 1)
            {
               if(param1.skyType == 0)
               {
               ...
               {
                  if(param1.currentFrameName != "普通武器跳击")
                  {
                     _loc3_ = "普通武器跳击";
                     this.changeActionByNameAndUpdate(param1,_loc3_);
                     return;
            ...
            else if(param1.skyType == 0)
            {
               if(param1.currentFrameName != "场景枪攻击")
               {
                  if(param1.gunslot.startGJ("场景枪攻击"))
                  {
                     _loc3_ = "场景枪攻击";
                     this.changeActionByNameAndUpdate(param1,_loc3_);
                     return;
            ...
            {
               if(param1.currentFrameName != "场景枪跳击")
               {
                  if(param1.gunslot.startGJ("场景枪跳击"))
                  {
                     _loc3_ = "场景枪跳击";
                     this.changeActionByNameAndUpdate(param1,_loc3_);
                     return;
         ...
         {
            if(param1.gunslot.gmUpdate(param1))
            {
               if(param1.currentFrameName == RAPao.name)
               {
                  this.changeActionByNameAndUpdate(param1,RAPao.name);
               }
            }
         }
      ...
      private function moveRun(param1:CPlayer) : void
      {
         ...
         while(_loc29_ > 0)
         {
            if(_loc3_ < 0)
            {
               if(GM.levelm.hitTestByRoleY(_loc24_,_loc25_ - GS.a120,0))
               {
                  break;
               }
      ...
      private function changeActionByNameAndUpdate(param1:CPlayer, param2:String) : void
      {
         ...
         if(param2 == "场景枪跳击" || param2 == "场景枪攻击")
         {
            curAction = param1.gunslot.getAction(param2);
         }
         else
         {
         ...
         if(param1.gunslot.getCurrentG() != 1)
         {
            if(this.tempMapGunNameArr.indexOf(param1.currentFrameName) == -1)
            {
               this.vp.enterActionChange(false);
            }
            else
            {
               this.vp.enterActionChange(true);
            }
         }
         curAction.gmUpdate(param1);
