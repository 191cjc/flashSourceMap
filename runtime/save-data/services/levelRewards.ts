...
export const SOLAR_PET_PASSIVE_SKILL_GROUP = 5;
export const SOLAR_PET_PASSIVE_SKILL_NAME = "永久霸体";
const SOLAR_PET_PASSIVE_ACTION_COOLDOWN = "999999999";
...
function solarPetActionRecord(
  ...
  options: { skillName?: string; bulletKey?: string | null; randomCd?: string; skillCd?: string } = {}
): string {
  ...
  record = replaceTagContent(record, "技能CD", options.skillCd ?? "0");
  ...
  if (bulletKey != null) {
    ...
    record = replaceTagContent(record, "生成子弹", "{}");
  }
...
function solarPetSkillName(group: number): string {
  if (group === SOLAR_PET_PASSIVE_SKILL_GROUP) {
    return SOLAR_PET_PASSIVE_SKILL_NAME;
  }
  return `技能${group}`;
}

function isSolarPetPassiveSkillGroup(group: number): boolean {
  return group === SOLAR_PET_PASSIVE_SKILL_GROUP;
}
...
function solarPetSkillRecord(group: number, quality: number): string {
  ...
  const passive = isSolarPetPassiveSkillGroup(group);
  return [
    "\t<宠物技能显示与说明>",
    ...
    `\t\t<等级成长的技能伤害系数>${passive ? "0" : solarPetSkillDamageValue(group, quality, SOLAR_PET_SKILL_GROWTH_DAMAGE_BY_GROUP)}</等级成长的技能伤害系数>`,
    `\t\t<基数>${passive ? "1" : solarPetSkillDamageValue(group, quality, SOLAR_PET_SKILL_BASE_DAMAGE_BY_GROUP)}</基数>`,
    passive ? "\t\t<技能说明>被动：永久霸体，免疫冰冻、石化、眩晕。稳定度Z%</技能说明>" : "\t\t<技能说明>对敌人造成Z%的伤害。</技能说明>",
    `\t\t<最低资质限制>${group}</最低资质限制>`,
    "\t</宠物技能显示与说明>",
  ].join("\n");
}
...
function isSolarPetSkillListId(value: string | null): boolean {
  ...
  return SOLAR_PET_SKILL_QUALITY_SUFFIXES.some((suffix) => {
    if (!value.endsWith(suffix)) {
      return false;
    }
    const name = value.slice(0, -suffix.length);
    return /^技能[1-7]$/.test(name) || name === SOLAR_PET_PASSIVE_SKILL_NAME;
  });
}

function isSolarPetSkillActionName(value: string | null): boolean {
  if (value == null || !/^[1-7]/.test(value)) {
    return false;
  }
  return isSolarPetSkillListId(value.slice(1));
}
...
function solarPetSkillActionRecord(template: string, pot: number, group: number, quality: number): string {
  ...
  if (isSolarPetPassiveSkillGroup(group)) {
    return solarPetActionRecord(template, "1", sourceActionLevel, {
      skillName: `${pot}${listId}`,
      bulletKey: null,
      randomCd: "0",
      skillCd: SOLAR_PET_PASSIVE_ACTION_COOLDOWN,
    });
  }
...
export function applySolarPetPatchToXml(
  ...
  for (let group = 1; group <= SOLAR_PET_SKILL_GROUP_COUNT; group += 1) {
    ...
    for (let quality = 0; quality < SOLAR_PET_SKILL_QUALITY_COUNT; quality += 1) {
      ...
      if (bulletTemplate != null && !isSolarPetPassiveSkillGroup(group)) {
...
export function getSolarPetPatchState(): SolarPetPatchState {
  ...
  try {
    ...
    const complete =
      ...
      bulletRecordCount >= 3 &&
      petBulletRecordCount >= (SOLAR_PET_SKILL_GROUP_COUNT - 2) * SOLAR_PET_SKILL_QUALITY_COUNT &&
